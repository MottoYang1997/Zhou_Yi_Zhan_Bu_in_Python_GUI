...
# Date- Oct 08, 2023
# Modified--Yiming Yang (MottoYangUESTC@Outlook.com)
# Date- March 24, 2025
...
msg_str += print_and_save_msg("“{:s}”的占卜结果:".format(motivation))

msg_str += print_and_save_msg("本卦为" + str(Ben_Gua))
...
if len_Bian_Yao >0:
     msg_str += print_and_save_msg("变卦:{}, \n变爻:{}".format(Bian_Gua, Bian_Yao))
else:
     msg_str += print_and_save_msg("无变卦")
...
msg_str += print_and_save_msg( '\n本卦为 {}，'.format(All_Gua_dic[Ben_Gua_key][0]))
if len_Bian_Yao >0:
    msg_str += print_and_save_msg('变卦为 {}， \n'.format(All_Gua_dic[Bian_Gua_key][0]))

msg_str += print_and_save_msg('解卦结果如下：\n')
...
showinfo(title="占卜结果",message=msg_str)
